...
class ByinAPIClient {
    ...
    async getAndCompleteTasksForUser(token) {
        ...
        for (const taskType of taskTypes) {
            ...
            try {
                this.log(`Getting task list for type ${taskType}...`, 'info');
                ...
                this.log(`Found ${unfinishedTasks.length} unfinished tasks for type ${taskType}.`, 'info');
                ...
                for (const task of unfinishedTasks) {
                    try {
                        ...
                        if (joinResponse.status === 200 && joinResponse.data.code === 0) {
                            this.log(`Successfully completed task ${task.taskName.yellow} (Type ${taskType}) | Reward: ${task.reward.toString().magenta}`, 'custom');
                        } else {
                            this.log(`Failed to complete task ${task.taskName} (Type ${taskType}): ${joinResponse.data.msg}`, 'error');
                        }
                    } catch (error) {
                        this.log(`Error executing task ${task.taskName} (Type ${taskType}): ${error.message}`, 'error');
                    }

                    await new Promise(resolve => setTimeout(resolve, 1000));
                }

                this.log(`Completed all unfinished tasks for type ${taskType}.`, 'success');
            } catch (error) {
                this.log(`Error retrieving or completing tasks of type ${taskType}: ${error.message}`, 'error');
            }

            await new Promise(resolve => setTimeout(resolve, 2000));
        }

        this.log(`Completed all task types.`, 'success');
    }
        ...
        while (true) {
            for (let i = 0; i < data.length; i++) {
                ...
                try {
                    ...
                    this.log(`Cannot check the proxy IP: ${error.message}`, 'warning');
                }

                console.log(`========== Account ${i + 1} | ${firstName.green} | IP: ${proxyIP} ==========`);
                
                this.log(`Logging into account ${userId}...`, 'info');
                ...
                if (loginResult.success) {
                    ...
                    if (loginResult.isNew === 1) {
                        ...
                        if (receiveResult.success) {
                            ...
                            this.log(`Could not receive currency: ${receiveResult.error}`, 'error');
                        }
                    } else {
                    ...
                    if (accountInfoResult.success) {
                        ...
                        this.log(`Could not retrieve account information: ${accountInfoResult.error}`, 'error');
                    }
